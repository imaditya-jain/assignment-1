...
export default async function POST(request: NextApiRequest, response: NextApiResponse) {
    ...
    try {
        ...
        if (!email) return response.status(400).json({ success: false, error: "Provide required fields." })
        ...
        const mail = {
            ...
            html: `<div><p>Enter <b>${OTP}</b> in the app to reset your password.</p></div>`
        }
        ...
        if (error instanceof Error) {
            console.log(`An error occurred while forgot password: ${error.message}`)
        } else {
